...
There are however some other interesting properties to this mechanism of representing state. As an example, it is possible to go back and look at what a state was at a given point in time. Consider for example, that the account has reached a balance below zero and there is a rule that says it is not supposed to. It is relatively easy to view the state the account was in just prior to processing the transaction that put it into the invalid state. This makes it far easier to reproduce what often times end up as heisenbugs in other circumstances.
